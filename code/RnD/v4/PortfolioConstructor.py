...
class PortfolioConstructor(nn.Module):
    ...
    def portfolio_creator(self, scores):
        ...
        if self.long_only:
            ...
            long_mask = torch.BoolTensor([i in long_sqs for i in range(rank.shape[0])]).to(self.device)
            long_scores = torch.where(long_mask, scores, torch.full_like(scores, -float('inf')))
            ...
            short_mask = torch.BoolTensor([i in short_sqs for i in range(rank.shape[0])]).to(self.device)
            short_scores = torch.where(short_mask, 1-scores , torch.full_like(scores, -float('inf')))
